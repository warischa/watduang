...
**Game screens** — a game renders no `<a href>` inside the play surface; a double-tap on a transition lands on it and leaves the round. The surface is `#stage` on a shell-rendered game, and the whole of `src/play/<id>/` on a play route — the directory **is** the boundary, because page chrome lives outside it: ADR-0014, retargeted by ADR-0041, re-scoped by ADR-0055. The one crawlable outbound link lives in static page chrome above the surface. What is load-bearing is that the link is static and sits outside the play surface — **not where it goes**, so no destination is written here — it has been retargeted more than once and a destination in prose has no gate. Read it off `GameLayout.astro` and `PlayExit.astro`. Two instruments, two owners: `scripts/no-nav-in-stage-check.mjs` scans both regions for literal anchors, and player names reaching the DOM are a separate, attacker-owned channel guarded at the escape sink by `src/play/name-escaping.test.mjs` (ADR-0026). Neither substitutes for the other. ⚠ The gate fleet reaches `src/play` as of gh#170 — but `arm-gate-coverage-check` asks only whether a route calls `armAllButtons` **somewhere**, never whether every reveal path is covered, and a route can hide bypasses behind its green. A route's own `arm-reveal-paths.test.mjs` is what closes that, and **every play route has one** (gh#170, extended per route since). No count is written here on purpose — a number in prose has no gate. Count it from disk when you need it. ⚠ **Closing a modal is itself a reveal** — the control behind it is enabled and its arm window long expired, which is exactly why a second contact fires it; three routes shipped comments arguing the opposite ("nothing is rebuilt, so nothing to re-arm"). Which CLOCK that gate reads is ADR-0059 — the browser's input stamp, never handler time, and the rule binds every file with its own timer on that constant, not the one a ruling happens to name. The whole class IS gated: ADR-0057 (accepted 2026-09-01) — every close, cancel and confirm path arms its region, and each route's `arm-reveal-paths.test.mjs` carries a close-path case (gh#187).

⚠ Before running a build · **deleting or renaming a page** · driving two headless probes at once · or putting a choice to the site owner → read `docs/runbook.md` first · verifying anything the way CI does, or reading CI's own verdict → `docs/agents/ci-verification.md` (a local green predicts CI only via `scripts/run-workflow-gates.sh`, never a hand-picked subset of gates; it also carries the per-step endpoint) · **before writing any verification command** → `docs/agents/shell-traps.md`, the ways a probe reports a number that is not true. **A documented command is copied from its opening line to its closing fence, never from a line window** — a slice keeps the command and drops the clauses that make it correct, then reports failures that are not real.

⚠ **The fast lane already exists — do not re-invent it, and do not hand-pick gates instead** → `docs/agents/ci-verification.md` § "The fast lane already exists — do not re-invent it, and do not hand-pick gates instead", for how it works, what CI decides by itself, and the two things that are NOT true about cutting scope.

⚠ **A brief's Premise cites the ticket's LAST comment, never a handoff's summary of it — and reads the body and that comment IN FULL, never a slice.** A comment states its problem at the top and its resolution at the bottom, so a `head` cut keeps the grievance and drops the fix; slice to shortlist, read whole before asking or briefing. A gate green on an apparent violation is a carve-out until its own comment says otherwise. A scout's map is not that reading — it locates code, never what the ticket last decided, and its honest "could not rule out" is not a green light. Read the sentence AFTER the one that proves you wrong, or a refusal over-reads a freeze the next line lifts. A comment asserting a set it never enumerates — "six decisions", "three bad numbers" — is not a premise: say it is unenumerated and ask for the list. How a label narrows what it licenses, and why `state: OPEN` proves nothing → `docs/agents/ticket-actionability.md`; the incidents behind each rule → `docs/sessions-archive.md`.

⚠ **A text grep cannot answer a question about a runtime value** → `docs/agents/shell-traps.md` § "The instrument measured something other than what you asked", last bullet — the `manifest.ts` case that proved it and what to execute instead.

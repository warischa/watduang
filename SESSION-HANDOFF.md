...
next:
- [ ] F1's **browser race window is unmeasured** — storage semantics probe-confirmed, but "module resolves before `location.reload()` commits" is argued from `65d3d3c`'s history, never observed · done when a CDP run shows the interleaving or proves it unreachable · **the harness must land IN the tree this time** · while it is up, re-capture `evidence/20/01-*` against HEAD — the committed capture predates `4b14565` (ADR-0009 provenance class)

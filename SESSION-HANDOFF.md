...
next:
- [ ] agent: gh#138 write the play-route recipe — its 3-games condition is now met; fold the paid-for traps: npm run build never npx astro build (prebuild owns the OG gate), inline handlers -> delegated listeners (power-meter pattern), the gate wiring list (freeze basenames + bytes, control-floor attribution, OG image), and no literal invisible bytes in source (an agent landed a real NUL this session)
...
- [ ] agent: move the reusable play-route browser probes from the session scratchpad into scripts/ — evidence outside the repo dies with the session

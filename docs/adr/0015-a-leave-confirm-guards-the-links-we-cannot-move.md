...
gh#106 raised a hole in the Decision's own predicate: the guard is appended to PlayerSetup's island and
is inert unless `#player-setup` exists, so a **panel-less page that starts a round** would ship
...
**The ruling: the current state is option 3 — the guard out of the panel island — by de facto.**
...
What did happen, through the gh#121 work, is that the guard stopped depending on the panel:
`leave-confirm-check` and `round-start-announce-check` both exit 0, and both name เซียมซี as the
panel-less page that starts a round. **Read their counts off the checks, never off prose** — gh#106's
2026-09-17 comment records today's, and that ticket's 2026-09-10 comment records smaller ones that went
stale when the tree gained a game.
...
probe cannot drive it. The invariant holds there by mounting rather than by driving — a genuine
coverage hole, not a by-design exemption. `round-start-announce-check` still reports its own limit
every run: presence of the channel, not proof it is written on the round-start path.

### The fact that would change this

A panel-less page that starts a round and does **not** mount `#leave-confirm` voids this amendment —
option 3 would have stopped being true de facto, and the choice between re-keying the carve-out and
making the mount structural would reopen. `leave-confirm-check` is what would catch it.

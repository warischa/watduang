...
Outcome per gap. The red-then-green evidence, the allowlist rationale, and the reproduced brace case
live in `docs/verification/adr-0045-latent-gaps-disposition.md` (moved there at an ADR-0012 task seam).

1. **FIXED** — the workflow reader inverts an unowned set: `SAFE_STEP_SIBLING_KEYS` is allowlisted and
   every other step sibling key routes the step into the guarded bucket, so it fails closed on keys
   GitHub adds later. **Bounded to STEP scope** — a job-level `if:`/`needs:`, an empty matrix, or a
   workflow-level `paths:` filter sits above step indent and stays invisible; `ci.yml` is single-job
   today and only today.
2. **FIXED** — a gate-inventory glob reports a gate present on disk but absent from both executors, and
   an exemption key matching no real file.
3. **FIXED** — the wrapper regex matches the script artifact anywhere in a command, not only after an
   invoker prefix.
4. **FIXED** — the entry-name lookahead no longer lets `npm run check:all` satisfy an entry named
   `check`.
5. **ACCEPTED as a disclosed ceiling** — braces inside string literals mis-scope
   `extractSelftestBody`, and a gutted selftest can be credited with a neighbouring function's
   assertions: a false green on the very counter that proves a gate earned its green. Accepted because
   no gate in this repo carries that shape today; recorded as unguarded, not as a non-issue.

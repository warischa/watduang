...
What a `ready-for-agent` label actually licenses, and why `state: OPEN` is not proof work is
undone: `docs/agents/ticket-actionability.md`.
...
**A citation's token is the issue number, never the เลขใบ.** Read it from
`gh issue view <n> --json number`, or off the issue URL — never from the front of the title, which is where the
เลขใบ sits. On 2026-09-22 six new code comments cited `gh#238` for issue #239; #238 is a real,
closed, unrelated ticket, so every one of them pointed a reader at the wrong work and every gate
stayed green. No gate reads a ticket number out of a comment or an issue body — this rule is the
only thing standing between the title and the citation.

...
const MEASURE = (primarySel, screenSel, firstItemSel, setSel) => `
  ${HELPERS}
  ...
  const setReadings = ${setSel ? `[...document.querySelectorAll(${JSON.stringify(setSel)})].map((e, i) => {
    const r = e.getBoundingClientRect();
    return { i, label: (e.textContent || '').trim().slice(0, 18), top: r.top, bottom: r.bottom,
             height: r.height, visiblePx: clip(r.top, r.bottom, 0, innerHeight),
             bottomPastFold: r.bottom - innerHeight };
  })` : 'null'};
  ...
  return {
    ...
    bottomPastFold: pRect.bottom - innerHeight,
    tallestBoxes: boxes,
    firstItemReading, setReadings,
  };
`;
...
const ROUTE_PATHS = {
  ...
  'short-stick': [
    { clickSel: null, primarySel: '#stick-grid', screenSel: '#view-draw', report: true,
      firstItemSel: '.straw-btn:not([disabled])', setSel: '.straw-btn',
  ...
  'pinocchio-luck': [
    ...
    { clickSel: '#ready', primarySel: '.answer', screenSel: '#stageFrame', report: true, name: 'answer-choice', setSel: '.answer',
  ...
  'wire-snip-panic': [
    { clickSel: null, primarySel: '#btn-trigger-scan', screenSel: '#screen-game', report: true,
      label: '#btn-trigger-scan', why: 'established primary for this route (182-320-fold).' },
  ],
  'how-close-is-near': [
...
async function measureOne(session, routeId, step) {
  const m = await session.evaluate(MEASURE(step.primarySel, step.screenSel, step.firstItemSel, step.setSel));
...
export default async function main(session) {
  ...
  for (const [routeId, path] of Object.entries(ROUTE_PATHS)) {
    if (ONLY.length && !ONLY.includes(routeId)) continue;

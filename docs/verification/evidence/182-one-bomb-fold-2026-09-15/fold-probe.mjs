// gh#182 evidence run: below-fold position of the primary control on wire-snip-panic (positive
// control, reproducing the recorded 182-320-fold figures) and one-bomb (new measurement) at 320x568,
// on the play screen the walk reaches after setup. Measurement only, no layout change. Copied
// verbatim from docs/verification/evidence/182-320-fold/fold-probe.mjs; the ROUTES constant below is
// the one edit this evidence run makes.
...
const ROUTES = [
  // Positive control: recorded in 182-320-fold/below-fold-primary-controls.md as rect.top 613 or
  // 640 (two-state screen), rect.bottom 712 or 739, 0 visible px. src/play/freeze-tap/ has had no
  // markup/CSS/main.ts change since the 182-320-fold capture (verified: git log 6dbbbb7..HEAD --
  // src/play/freeze-tap/ touches only a test file), so this route's recorded figures are still live.
  ...
  // NOTE: wire-snip-panic's recorded figures in 182-320-fold ARE STALE — commit 8db9bd2 (2026-09-13,
  // after the 182-320-fold capture) moved this route's primary control to fix the gh#182 violation.
  // Measured here for completeness; not used as the positive control because the reference document
  // predates that fix. Its numbers below should match commit 8db9bd2's own cited figures (top 540,
  // bottom 616, 28px visible), not the older below-fold-primary-controls.md row.
  ...
  // one-bomb: container/primary/block confirmed against src/play/one-bomb/markup.html + main.ts.
  // #startPlayBtn is "the button that starts the game" per overrides.css's own comment; #ob-players
  // is the seat-row list that grows with roster size and is the block pushing the button down.
  { id: 'one-bomb', container: 'div#menuOverlay.menuOverlay', primary: '#startPlayBtn', block: '#ob-players', loads: Number(process.env.OB_LOADS || 3) },
];

...
// aria-label, and the stepper display above the list is a bare number too. A digit-scanning check
// would red on both. Equality with the cast is immune to the whole class.
//
...
//
// This route is the one whose setup rows do not exist in markup.html at all -- the mockup ships a
// player-count stepper and no name fields -- so main.ts is the only place the row can be read from.
...
const runRender = new Function(
  'document', '$', 'currentCount', 'mascotEmoji', 'names', 'NAME_MAX', 'save',
  `${body}\nrenderRows();`,
);
...
function render(count) {
  ...
  const listEl = el('ob-players');
  runRender(
    document,
    (id) => (id === 'ob-players' ? listEl : el(id)),
    () => count,
    mascotEmoji,
...
/** The badge inside one row: the single child carrying the seat class the stylesheet paints. */
function badge(row) {
  const found = row.children.find((c) => c.className === 'ob-player-seat');
...
test('gh#209: the row keeps its POSITIONAL numbered label, which this check must never flag', () => {
  const rows = render(4);
  rows.forEach((row, i) => {
    const input = row.children.find((c) => c.className === 'ob-player-name');
...
// shared roster never receives a name longer than the field advertises.
test('the name field caps its own length', () => {
  for (const row of render(2)) {
    const input = row.children.find((c) => c.className === 'ob-player-name');

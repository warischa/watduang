// gh#176, the pattern gh#174 set for this file: after the confirm is accepted, the roster holds the
// animal cast again, it holds exactly as many seats as before, and every name a player typed is gone.
//
// It runs the REAL bytes. main.js is a lifted IIFE with no exports, so resetPlayerNames is sliced out
// by source text and evaluated over a `game` object this file supplies -- the same technique
// short-stick/reset-names.test.mjs uses. A rename or a rewrite of resetPlayerNames fails this file
...
// state move and its handler owns saveSettings/renderSetupPlayerList. What that costs is stated: this
// proves the WIPE, never that the button is wired to it. The wiring (trigger -> modal -> confirm
// button -> resetPlayerNames) is pinned by arm-reveal-paths.test.mjs, and neither file substitutes
// for the other.
...
// the state array it boots with, the blank-field fallback and the added seat; all three now route
// through the shared cast. Pinned by absence, which is fragile on its own, so the positive half is
// pinned too: the cast import has to still be there.
